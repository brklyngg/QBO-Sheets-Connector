...
function scheduledDatasetRun(e) {
  ...
  try {
    ...
    const realmId = getUserProps().getProperty('QBO_REALM_ID');
...
function notifyScheduleError(datasetId, error) {
  try {
    ...
    const notificationEnabled = getUserProps().getProperty('SCHEDULE_ERROR_NOTIFICATIONS') === 'true';
